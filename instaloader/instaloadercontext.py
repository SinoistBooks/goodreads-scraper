...
class InstaloaderContext:
    ...
    def __init__(self, sleep: bool = True, quiet: bool = False, user_agent: Optional[str] = None,
        ...
        self._rate_controller = rate_controller(
            self) if rate_controller is not None else RateController(self)
    ...
    def get_anonymous_session(self) -> requests.Session:
        ...
        session.headers.update(
            self._default_http_header(empty_session_only=True))
        # Override default timeout behavior.
        # Need to silence mypy bug for this. See: https://github.com/python/mypy/issues/2427
    ...
    def save_session_to_file(self, sessionfile):
        ...
        pickle.dump(requests.utils.dict_from_cookiejar(
            self._session.cookies), sessionfile)
    ...
    def login(self, user, passwd):
        ...
        if 'authenticated' not in resp_json:
            ...
            if 'message' in resp_json:
                raise ConnectionException(
                    "Login error: Unexpected response, \"{}\".".format(resp_json['message']))
            else:
                raise ConnectionException(
        ...
        session.headers.update({'X-CSRFToken': login.cookies.get('csrftoken')})
        self._session = session
        self.username = user
    ...
    def two_factor_login(self, two_factor_code):
        ...
        login = session.post('https://www.instagram.com/accounts/login/ajax/two_factor/',
                             data={
                                 'username': user, 'verificationCode': two_factor_code, 'identifier': two_factor_id},
    ...
    def get_json(self, path: str, params: Dict[str, Any], host: str = 'www.instagram.com',
        ...
        try:
            ...
            resp = sess.get(
                'https://{0}/{1}'.format(host, path), params=params, allow_redirects=False)
            if resp.status_code in self.fatal_status_codes:
                redirect = " redirect to {}".format(
                    resp.headers['location']) if 'location' in resp.headers else ""
            ...
            if is_html_query:
                ...
                post_or_profile_page = list(entry_data.values())[
                    0] if entry_data is not None else None
                if post_or_profile_page is None:
                    raise ConnectionException(
                        "\"window._sharedData\" does not contain required keys.")
            ...
            if 'status' in resp_json and resp_json['status'] != "ok":
                if 'message' in resp_json:
                    ...
                    raise ConnectionException(
                        "Returned \"{}\" status.".format(resp_json['status']))
            return resp_json
    ...
    def get_raw(self, url: str, _attempt=1) -> requests.Response:
        ...
        if resp.status_code == 200:
            ...
            if resp.status_code == 403:
                ...
                raise QueryReturnedForbiddenException(
                    "403 when accessing {}.".format(url))
            if resp.status_code == 404:
                # 404 not worth retrying.
                raise QueryReturnedNotFoundException(
                    "404 when accessing {}.".format(url))
            raise ConnectionException(
                "HTTP error code {}.".format(resp.status_code))
...
class RateController:
    ...
    def _dump_query_timestamps(self, current_time: float, failed_query_type: str):
        ...
        for query_type, times in self._query_timestamps.items():
            ...
            self._context.error(" {} {:>32}: {}".format(
                ...
                query_type,
                " ".join("{:4}".format(reqs)
                         for reqs in reqs_in_sliding_window)
            ), repeat_at_end=False)
    ...
    def _reqs_in_sliding_window(self, query_type: Optional[str], current_time: float, window: float) -> List[float]:
        if query_type is not None:
            ...
            relevant_timestamps = [t for times in (
                tp[1] for tp in graphql_query_timestamps) for t in times]
        ...
        def untracked_next_request_time():
            if untracked_queries:
                if query_type == "iphone":
                    ...
                    self._iphone_earliest_next_request_time = min(
                        reqs_in_sliding_window) + iphone_sliding_window + 18
                else:
                    reqs_in_sliding_window = self._reqs_in_sliding_window(query_type, current_time,
                                                                          per_type_sliding_window)
                    self._earliest_next_request_time = min(
                        reqs_in_sliding_window) + per_type_sliding_window + 6
